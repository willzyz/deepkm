  ...
  def build_projector(self):
    with tf.variable_scope("projector"):
      with tf.variable_scope("l1"):
        self.l1_lin = linear(tf.expand_dims(self._Xc, 0), self.projector_embed_dim, bias=True)
        self.l1 = tf.nn.relu(self.l1_lin)

      with tf.variable_scope("l2"):
        self.l2_lin = linear(self.l1,	 
      		            self.projector_embed_dim, bias=True)
        self.l2 = tf.nn.relu(self.l2_lin)

      with tf.variable_scope("mu"):
        self.mu = linear(self.l2, self.n_topics, bias=True)

      with tf.variable_scope("log_sigma_sq"):
        self.log_sigma_sq = linear(self.l2, self.n_topics, bias=True)
